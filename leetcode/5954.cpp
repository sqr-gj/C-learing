...
//题解：https://leetcode-cn.com/problems/watering-plants-ii/solution/shuang-zhi-zhen-mo-ni-by-cao-mu-hui-d-6aew/

//                                          2021/12/12
int question5954(vector<int>& plants, int capacityA, int capacityB) {
    int left = 0;
    ...
    while (left <= right) {
        if (left == right) {
            if (A_res < plants[left] && B_res < plants[left])
                ++cnt;
        }
        else {
            if (A_res < plants[left]) {
                ++cnt;
                A_res = capacityA - plants[left];
            }
            else {
                A_res = A_res - plants[left];
            }

            if (B_res < plants[right]) {
                ++cnt;
                B_res = capacityB - plants[right];
            }
            else {
                B_res = B_res - plants[right];
            }
        }
        ++left;
        --right;
