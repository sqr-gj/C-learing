...
vector<int> combine_nums (string s) {

}

//未完成                           2021/10/31
bool possiblyEquals(string s1, string s2) {
    vector<int> possible_len1;
    vector<int> possible_len2;
    int left = 0;
    int right = 0;
    int sz1 = s1.size();
    int sz2 = s2.size();
    while (right < sz1) {
        if (isalpha(s1[right])) {
            if (isdigit(s1[left])) {
                auto v = combine_nums(string(s1.begin() + left, s1.begin() + right));
                if (possible_len1.empty())
                    possible_len1 = v;
                else {
                    int sz = possible_len1.size();
                    for (int i = 0; i < sz; ++i) {
                        int pre_num = possible_len1[i];
                        for (auto &num : v)
                            possible_len1.push_back(pre_num + num);
                    }
                    possible_len1.erase(possible_len1.begin(), possible_len1.begin() + sz);
                }
            }
        }
        else {
